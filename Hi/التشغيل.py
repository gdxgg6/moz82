...
from Hi.queues import QUEUE, add_to_queue, get_queue, clear_queue
from Hi.decorators import authorized_users_only
from Hi.handlers import skip_current_song, skip_item
from pyrogram.errors import FloodWait, MessageNotModified
from youtubesearchpython import SearchVideos
from yt_dlp import YoutubeDL
from Hi.merrors import capture_err
...
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("𝘽𝙀𝙄𝙉𝙂 𝙊𝙋𝙀𝙍𝘼𝙏𝙄𝙊𝙉𝘼𝙇 𝘼𝙉𝘿 𝙋𝙍𝙊𝘾𝙀𝙎𝙎𝙀𝘿...")
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_video(
                    video="https://telegra.ph/file/82b9c80a12c945db7e3ad.mp4",
                    caption=f"""
᥉ꪮꪀᧁ  : [{songname}]({link})
Ꭵძ : {chat_id}
ᠻꪮɾꪔ : {m.from_user.mention} .
""",                )
            else:
...
@Client.on_message(filters.command(["ش_فيد"], prefixes=f"{HNDLR}"))
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("تم سكتت اسف @qokkk [.](https://telegra.ph/file/56f4b50f484dc7e163728.mp4) ")
        except Exception as e:
