...
@Client.on_message(filters.user(OWNER_ID) & filters.command(['بنك'], prefixes=f"{HNDLR}"))
async def ping(client, m: Message):
   start = time()
   current_time = datetime.utcnow()
   m_reply = await m.edit("`لحضه...`")
...
@Client.on_message(filters.user(OWNER_ID) & filters.command(['خيط'], prefixes=f"{HNDLR}"))
async def pong(client, m: Message):
   start = time()
   current_time = datetime.utcnow()
   pong = await m.edit("`اه...`")
   delta_ping = time() - start
   await pong.edit("8✊===D")
   await pong.edit("8=✊==D")
   await pong.edit("8==✊=D")
...
@Client.on_message(filters.user(OWNER_ID) & filters.command(['اوامر'], prefixes=f"{HNDLR}"))
...
@Client.on_message(filters.user(OWNER_ID) & filters.command(['سورس'], prefixes=f"{HNDLR}"))
