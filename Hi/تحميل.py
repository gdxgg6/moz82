...
@Client.on_message(filters.command(["ن", "ح"], prefixes=f"{HNDLR}"))
async def song(client, message: Message):
    urlissed = get_text(message)
    if not urlissed:
        await client.send_message(            message.chat.id,            "صيغة الأمر غير صالحة .",        )
        return
    pablo = await client.send_message(message.chat.id, f"@qokkk جاي يحمل انتضر [.](https://telegra.ph/file/3080cf0a8120db0b574b9.mp4)`{urlissed}`")
...
@Client.on_message(filters.command(["نزل_فيد", "ح_فيد"], prefixes=f"{HNDLR}"))
async def vsong(client, message: Message):
    urlissed = get_text(message)

    pablo = await client.send_message(message.chat.id, f" يبحث [.](https://telegra.ph/file/733cc4e44a383f9e9f8ce.mp4) `{urlissed}`")
    if not urlissed:
